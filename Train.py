...
from lib.Network_ResNet50 import *
# from lib.NetWork_Res2Net50 import *
# from lib.Network_PVT import *
...
#                             structure_loss(preds[3], gts)*0.5 + structure_loss(preds[5], gts)*1
...
#                             weighted_structure_loss(preds[3], gts)*0.5 + weighted_structure_loss(preds[5], gts)*1
...
#                         partial_cross_entropy(preds[3], scribble.unsqueeze(1))*0.5 + partial_cross_entropy(preds[5], scribble.unsqueeze(1))*1
    ...
    try:
        for i, (images, gts, scribble, gt_smalls) in enumerate(train_loader, start=1):
            ...
            if opt.lossmanner == 'NoWeighted':
                ...
                loss_body = (structure_loss(preds[1], gts) * 0.125 +
                             structure_loss(preds[2], gts) * 0.25 +
                             structure_loss(preds[3], gts) * 0.5 +
                             structure_loss(preds[5], gts) +
                             structure_loss(preds[6][0], gts) * 0.125 +
                             structure_loss(preds[6][1], gts) * 0.125 +
                             structure_loss(preds[6][2], gts) * 0.125 +
                             structure_loss(preds[6][3], gts) * 0.125)
            ...
            if opt.lossmanner == 'Weighted':
                ...
                loss_body = (weighted_structure_loss(preds[1], gts) * 0.125 +
                             weighted_structure_loss(preds[2], gts) * 0.25 +
                             weighted_structure_loss(preds[3], gts) * 0.5 +
                             weighted_structure_loss(preds[5], gts) * 0.5 +
                             weighted_structure_loss(preds[6][0], gts) * 0.125 +
                             weighted_structure_loss(preds[6][1], gts) * 0.125 +
                             weighted_structure_loss(preds[6][2], gts) * 0.125 +
                             weighted_structure_loss(preds[6][3], gts) * 0.125)
            ...
            # PCE loss
            loss_init1 = partial_cross_entropy(preds[0], scribble.unsqueeze(1)) * 0.0625
            loss_body1 = (partial_cross_entropy(preds[1], scribble.unsqueeze(1)) * 0.125 +
                          partial_cross_entropy(preds[2], scribble.unsqueeze(1)) * 0.25 +
                          partial_cross_entropy(preds[3], scribble.unsqueeze(1)) * 0.5 +
                          partial_cross_entropy(preds[5], scribble.unsqueeze(1)) * 0.5 +
                          partial_cross_entropy(preds[6][0], scribble.unsqueeze(1)) * 0.0625 +
                          partial_cross_entropy(preds[6][1], scribble.unsqueeze(1)) * 0.0625 +
                          partial_cross_entropy(preds[6][2], scribble.unsqueeze(1)) * 0.0625 +
                          partial_cross_entropy(preds[6][3], scribble.unsqueeze(1)) * 0.0625)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/hy-tmp/C/TrainDataset/',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='/hy-tmp/C/TestDataset/CAMO/',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str,
                        default='./snap_10_384_迭代/',
    ...
    #network
    # model = torch.nn.DataParallel(Network_interFA_noSpade_noEdge_ODE_slot_channel4(c  hannels=128), device_ids=device_ids)
    model = torch.nn.DataParallel(Network_interFA_noSpade_noEdge_ODE_slot_Channel4_Scribble(channels=128), device_ids=device_ids)
    ...
    """
     Scribble作为SAM的提示，利用SAM生成的伪标签对训练模型进行监督 
    """

    # # image-level selection   有图像筛选
    # train_loader = get_loader_scribble_noEdge_3326(
    ...
    train_loader = get_loader_scribble_noEdge(image_root=opt.train_root + 'Imgs/',
                              gt_root='/hy-tmp/C/TrainDataset/Combination/', #这里跑弱监督时，需要修改路径为SAM的分割结果
                              scribble_root=opt.train_root + 'Scribble/',
                              batchsize=opt.batchsize,
                              trainsize=opt.trainsize,
                              num_workers=8)
