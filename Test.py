...
parser.add_argument('--pth_path', type=str, default='/hy-tmp/WS/snap_10_384_迭代/Net_epoch_best.pth')
opt = parser.parse_args()

for _data_name in ['CAMO', 'COD10K', 'CHAMELEON', 'NC4K']:
    data_path = '/hy-tmp/C/TestDataset/{}/'.format(_data_name)
    save_path = './res_10_384/{}/{}/'.format(opt.pth_path.split('/')[-2], _data_name)

    # model = Network_interFA_noSpade_noEdge_ODE_slot_channel4(channels=128) #can be different under diverse backbone
    model = Network_interFA_noSpade_noEdge_ODE_slot_Channel4_Scribble(channels=128)
