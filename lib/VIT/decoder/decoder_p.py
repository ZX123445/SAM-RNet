...
class BiasFree_LayerNorm(nn.Module):#无偏置归一化
    ...
    def forward(self, x):
        sigma = x.var(-1, keepdim=True, unbiased=False) #计算输入x在最后一个维度上的方差
        return x / torch.sqrt(sigma+1e-5) * self.weight

class WithBias_LayerNorm(nn.Module):  #有偏置归一化
    def __init__(self, normalized_shape):
...
class LayerNorm(nn.Module):  #普通归一化
    def __init__(self, dim, LayerNorm_type):#dim：归一化的维度；LayerNorm_type：指定使用哪一种形式的归一化
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, mode):
        ...
        self.num_heads = num_heads #自注意力的头数
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1)) #一个可学习的参数，用于缩放注意力分数
...
class MSA_head(nn.Module):#处理具有不同注意力焦点的输入
...
class MSA_module(nn.Module):
    def __init__(self, dim=128):
        ...
        self.B_TA = MSA_head()  #处理背景的注意力
        self.F_TA = MSA_head()  #处理前景的注意力
        self.TA = MSA_head()   #处理整体结构的注意力
